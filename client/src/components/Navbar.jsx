        ...
        ${location.pathname === "/" && "bg-light"}`}>
            ...
            <Link to='/'>
                <motion.img whileHover={{ scale: 1.05 }}
                    src="/CarDekho.png"
                    alt="logo"
                    className="h-16 md:h-18 bg-gray-800 rounded-full"
                />
            </Link>
            ...
            ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
                ...
                <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                    <button onClick={() => isOwner ? navigate('/owner') : changeRole()} className='cursor-pointer'>{isOwner ? 'Dashnoard' : 'Listcars'}</button>
