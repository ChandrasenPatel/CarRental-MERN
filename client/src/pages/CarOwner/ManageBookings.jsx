...
const ManageBookings = () => {
    ...
    const fetchOwnerBoookings = async () => {
        try {
            const { data } = await axios.post('/api/bookings/owner')
                            ...
                            <tr key={index} className='border-t border-borderColor text-gray-500'>
                                ...
                                <td className='p-3'>
                                    {booking.status === 'pending' ? (
                                        <select onChange={(e) => changeBookingStatus(booking._id, e.target.value)} className='px-2 py-1.5 mt-1 text-gray-500
